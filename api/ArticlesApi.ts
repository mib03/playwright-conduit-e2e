...
export class ArticlesApi {
  ...
  async create(article: ArticleInput): Promise<Article> {
    ...
    if (response.status() !== 201) {
      throw new Error(
        `Article creation failed with HTTP ${response.status()}: ${await response.text()}`,
      );
    }
  ...
  async get(slug: string): Promise<Article> {
    const response = await this.request.get(
      `${this.baseUrl}/api/articles/${slug}`,
      {
        headers: { Authorization: `Token ${this.token}` },
      },
    );
    ...
    if (response.status() !== 200) {
      throw new Error(
        `Article retrieval failed with HTTP ${response.status()}: ${await response.text()}`,
      );
    }
    const body = (await response.json()) as { article: Article };
    return body.article;
  ...
  async delete(
    ...
    if (response.status() !== 204) {
      throw new Error(
        `Article deletion failed with HTTP ${response.status()}: ${await response.text()}`,
      );
    }
  }
}
